...
void testProperties() {
    ...
    TEST("test checkAndGet", [] {
        fs::path path{"../test-resources/properties/sample.properties"};
        
        auto props = loadProperties(path);
        
        assertThrows([&]() { props.checkAndGet("thisKeyDoesNotExists", "A new simple message"); });
        
        try {
            props.checkAndGet("thisKeyDoesNotExists", "A new simple message");
        } catch (Exception ex) {
            assertEquals("A new simple message", ex.what());
        }
    });
}
}
